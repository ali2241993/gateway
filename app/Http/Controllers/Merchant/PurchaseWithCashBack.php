...
class PurchaseWithCashBack extends Controller{
    public function purchaseWithCashBack(Request $request){
        try{
            $validator = Validator::make($request->all(), [
                ...
                'cashBackAmount'         => 'required|regex:/^[0-9]+(\.[0-9][0-9]?)?$/',
            ...
            $response = Http::post('http://127.0.0.1:8001/api/merchant/purchaseWithCashBack',$request->all());
            return $response;
        }
        catch(\exception $e){
            return $e;
